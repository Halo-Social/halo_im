...
struct DecodedMessageWrapper {
    ...
    static func encodeToClass(_ model: XMTP.DecodedMessage, client: Client) throws -> IMDecryptedMessage {
        let content = try ContentJson.fromEncoded(model.encodedContent, client: client).toJsonMap()
        let message = IMDecryptedMessage(
            id: model.id,
            topic: model.topic,
            contentTypeId: model.encodedContent.type.description,
            content: content,
            senderAddress: model.senderAddress,
            sent: Int64(model.sent.timeIntervalSince1970 * 1000),
            deliveryStatus: model.deliveryStatus.rawValue.uppercased()
        )
        return message;
    }
